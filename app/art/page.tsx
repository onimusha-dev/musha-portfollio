...
function GalleryImage({ piece, onClick }: { piece: Piece, onClick: () => void }) {
    ...
    return (
        ...
        >
            ...
            {/* hover overlay — only show if loaded */}
            {!isLoading && !isError && (
                <div
                ...
                >
                    ...
                    <p className="text-white/60 text-[11px] font-mono select-none">{piece.medium} · {piece.year}</p>
                </div>
            )}
        </div>
    );
...
function Lightbox({
    ...
    useEffect(() => {
        setIsLoading(true); // reset on piece change
    }, [piece.src]);
...
export default function ArtPage() {
    ...
    const prev = useCallback(() => setSelectedIdx(i => i !== null ? (i - 1 + GALLERY.length) % GALLERY.length : 0), []);
    const next = useCallback(() => setSelectedIdx(i => i !== null ? (i + 1) % GALLERY.length : 0), []);
